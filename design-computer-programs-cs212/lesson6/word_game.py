...
def readwordlist(filename):
    """Return a pair of sets: all the words in a file,
    and all the prefixes. (Uppercased.)
    """
        ...
        if isinstance(sq, set):
            ...
            if pre:  # Add to the letters already on the board
                ...
                for pre in find_prefixes(hand):
                    if len(pre) <= maxsize:
                        start = i - len(pre)
                        add_suffixes(removed(hand, pre),
                                     pre, start, row, results,
                                     anchored=False)
    return results
    ...
    for (i, sq) in enumerate(row[1:-1], 1):
        ...
        if sq == '*' or (is_empty(sq) and any(map(is_letter, neighborlist))):
            if is_letter(N) or is_letter(S):
                ...
                row[i] = anchor(L for L in LETTERS
                                if w.replace('.', L) in WORDS)
...
def cross_word_score(board, L, pos, direction):
    """Return the score of a word made in
    the other direction from the main word.
    """
    i, j = pos
