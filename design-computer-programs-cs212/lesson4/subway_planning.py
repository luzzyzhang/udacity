...
def subway(**lines):
    """Define a subway map.
    Input is subway(linename='station1 station2...'...).
...
boston = subway(
    blue=('bowdoin government state aquarium maverick ') +
         ('airport suffolk revere wonderland'),
    orange=('oakgrove sullivan haymarket state downtown ') +
         ('chinatown tufts backbay foresthills'),
    green=('lechmere science north haymarket government ') +
         ('park copley kenmore newton riverside '),
    red=('alewife davis porter harvard central mit ') +
    ('charles park downtown south umass mattapan')
)


def ride(here, there, system=boston):
    "Return a path on the subway system from here to there."
    return shortest_path_search(here, lambda s: system[s],
                                lambda s: s == there)
...
def test_ride():
    ...
    assert ride('mattapan', 'foresthills') == [
        'mattapan', 'red', 'umass', 'red', 'south',
        'red', 'downtown', 'orange', 'chinatown', 'orange',
        'tufts', 'orange', 'backbay', 'orange', 'foresthills']
    assert ride('newton', 'alewife') == [
        'newton', 'green', 'kenmore', 'green', 'copley',
        'green', 'park', 'red', 'charles', 'red',
        'mit', 'red', 'central', 'red', 'harvard', 'red',
        'porter', 'red', 'davis', 'red', 'alewife']
    assert (path_states(longest_ride(boston)) == [
        'wonderland', 'revere', 'suffolk', 'airport', 'maverick',
        'aquarium', 'state', 'downtown', 'park', 'charles',
        'mit', 'central', 'harvard', 'porter', 'davis', 'alewife'] or
        path_states(longest_ride(boston)) == [
            'alewife', 'davis', 'porter', 'harvard',
            'central', 'mit', 'charles',  'park',
            'downtown', 'state', 'aquarium', 'maverick',
            'airport', 'suffolk', 'revere', 'wonderland'])
...
def test_subway():
    assert boston['bowdoin'] == {'government': 'blue'}
    assert boston['mit'] == {'central': 'red', 'charles': 'red'}
    print 'subway func tests pass'
