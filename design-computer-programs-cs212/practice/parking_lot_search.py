...
A '|' represents a wall around the parking lot, a '.' represents an empty
square, and a letter or asterisk represents a car.  '@' marks a goal square.
...
where 'start' is the initial state of the puzzle and 'N' is the length of a
side of the square that encloses the pieces (including the walls, so N=8 here).
...
def psuccessors(state):
    """State is a tuple of (('c': sqs), ...); return a {state: actions}
    dict where action is of form ('c', dir), where dir is +/-1 or +/-N.
    """
    results = {}
        ...
        if c not in '|@':  # Walls and goals can't move
            ...
            for (d, start) in [(diff, max(sqs)), (-diff, min(sqs))]:
                for i in range(1, N-2):
                    ...
                    new_state = update(state, c, tuple(q+d*i for q in sqs))
                    results[new_state] = (c, d*i)
    return results
...
def locs(start, n, incr=1):
    # Return a tuple of n locations,
    # starting at start and incrementing by incr.
...
def grid(cars, N=N):
    ...
    goals = ((N ** 2) // 2 - 1,)
    walls = (locs(0, N) + locs(N*(N-1), N) +
             locs(N, N-2, N) + locs(2*N-1, N-2, N))
...
def test_parking():
    ...
    assert same_state(
            ...
            (('*', (25, 26)), ('B', (19, 27, 35)), ('P', (36, 37, 38)),
             ('O', (45, 53)), ('Y', (49, 50, 51)),
             ('|', (0, 1, 2, 3, 4, 5, 6, 7, 56, 57, 58, 59, 60, 61, 62,
                    63, 8, 16, 24, 32, 40, 48, 15, 23, 39, 47, 55)),
             ('@', (31,))))
    print 'test parking pass'
...
def valid_solution(puzzle, length):
    ...
    return (len(path_actions(path)) == length and same_state(path[0], puzzle)
            and is_goal(path[-1]) and
